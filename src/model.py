...
class WKV_6(torch.autograd.Function):
    ...
    def forward(ctx, B, T, C, H, r, k, v, w, u):
        with torch.no_grad():
            assert r.dtype == k.dtype == v.dtype == w.dtype == u.dtype == torch.bfloat16
            assert r.is_contiguous() and k.is_contiguous() and v.is_contiguous() and w.is_contiguous() and u.is_contiguous()
    ...
    def backward(ctx, gy):
        with torch.no_grad():
            ...
            gr = torch.empty((B, T, C), device=gy.device, dtype=torch.bfloat16, memory_format=torch.contiguous_format)
            gk = torch.empty((B, T, C), device=gy.device, dtype=torch.bfloat16, memory_format=torch.contiguous_format)
            gv = torch.empty((B, T, C), device=gy.device, dtype=torch.bfloat16, memory_format=torch.contiguous_format)
            gw = torch.empty((B, T, C), device=gy.device, dtype=torch.bfloat16, memory_format=torch.contiguous_format)
            gu = torch.empty((B, C), device=gy.device, dtype=torch.bfloat16, memory_format=torch.contiguous_format)
            wkv6_cuda.backward(B, T, C, H, r, k, v, w, u, gy, gr, gk, gv, gw, gu)
            return None, None, None, None, gr, gk, gv, gw, gu.sum(0).view(H, C//H)
